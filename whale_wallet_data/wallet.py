        ...
        if "$wallet" in message.content:
            ...
            if len(args) < 2:
                ...
                try:
                    ...
                    reflections = ((await market_api.get_dex_total_vol(True) * bal) / (20 * 550000000000))
...
async def liquidity_wallet_transfer():
    ...
    try:
        json = requests.get("https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress"
                            "=0x42981d0bfbaf196529376ee702f2a9eb9092fcb5&address=0x42981d0bfbaf196529376ee702f2a9eb9092fcb5"
                            "&tag=latest&apikey=" + API_KEY).json()
        if str(json.get('result')) == "":
            return ""

        ind = 11 - (20 - len(str(json.get('result'))))

        return 500000000 - int(str(json.get('result'))[:ind])
    except ValueError:
        return ""
...
async def whale_processing():
    ...
    while not bot.is_closed():
        ...
        API_KEY, addresses = "MJYCR4Q2Z98ZDHTWAMFFCTJXTEKEDPKFQQ", file.read().splitlines()
        try:
