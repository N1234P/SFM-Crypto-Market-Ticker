...
async def display_all_stats(ctx, ctx2, ctx3):
    ...
    transfer = price_formatter(await wallet.liquidity_wallet_transfer()) + " tokens"
    ...
    em.add_field(name="<:sfm:824031099402321961> SFM V2 Price (SFS BNB | BUSD)",
    ...
    em.add_field(name="💰 Reflections (Per Million Tokens - Estimate)", value=price_formatter(reflections) + " tokens ($" +
                                                                   price_formatter(reflections_USD) + ")")
    em.add_field(name="Liquidity", value="SFS: " + price_formatter(sms) + " BNB ($" + price_formatter(usd_sms) + ")\nBUSD: " + price_formatter(sms_busd))
    em.add_field(name="<:uptrend:838343716276142080> 24HR Percentage Change", value=percent)
    em.add_field(name="<:pandaBags:840623325909745674> Remaining Tokens For Automated Liquidity Transfer",
                 value=transfer)
    em.add_field(name="Other Coins",
                 value="$" + price_formatter(
                     await market_api.get_bitcoin_price("bitcoin")) + " (Bitcoin)\n" + "$" + price_formatter(
                     await market_api.get_bnb_price("binancecoin")) + " (BNB)")
    ...
    # ------------------------------------------ sfm international
    em.remove_field(0)
    em.insert_field_at(0, name="SFM Price (V2)", value="$" + sfm_swap[0])
    em.remove_field(8)
    em.insert_field_at(8, name="<:uptrend:896236524000129064> 24HR Percentage Change", value=percent)
    em.remove_field(9)
    em.insert_field_at(9, name="<:pandabags:896594881089261610> Remaining Tokens For Automated Liquidity Transfer",
                       value=transfer)

    await ctx3.send(embed=em, delete_after=240)
...
async def display_nonembedded_stats(ctx):
    ...
    if safemoonswap[0] == None or safemoonswap[0] == "":
        return False
    ...
    await ctx.send(
        "🌙 SFM Price V2 (SFS) | " + "$" + safemoonswap[0] + "\n\n🔥 24 Hour Burn | " + burn
        ...
        + "\n\n😁 Remaining Tokens For Automated Liquidity Transfer | " + transfer
        + "\n\n🪙 $" + price_formatter(
            await market_api.get_bitcoin_price("bitcoin")) + " (Bitcoin)" + "\n\n🪙 $" + price_formatter(
            await market_api.get_bnb_price("binancecoin")) + " (BNB)")
    return True
...
async def help(ctx):
    ...
    em.add_field(name="$%/$percent", value="View 24 hour percent changes for sfm/btc/bnb")
    ...
    em.add_field(name="$wallet <wallet address>",
                 value="Dm this bot in this format and you'll get the wallet's total tokens, USD equivalent, and "
                       "24 hour reflections")
    em.add_field(name="$transfer or $injection", value="Used to get the remaining tokens left for a v1 liquidity "
                                                       "transfer to v2")
