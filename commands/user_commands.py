...
async def cmds(message):
    ...
    if str(message.channel) == "⏫│fb-pricefeed":
    ...
    if str(message.channel) in channels or (
        ...
        if "$p" in message.content and all(words not in message.content for words in arr) or "$v2" in message.content:
            ...
            sms, sms2 = await get_sms_price(), await get_bnb_sms_price()

            description = "$" + str(sms2[0]) + " " + sms2[1] + "| $" + str(sms[0]) + " " + sms[1]

            await message.channel.send(
                embed=embedder("V2 (SFS BNB | BUSD)", description))
        ...
        if "$pcs" in message.content:
            ...
            description = "$" + price_formatter(await market_api.get_bnb_price("binancecoin"))
            await message.channel.send(
        ...
        elif "$%" in message.content or "$percent" in message.content:
            sfm_percent, btc_percent, bnb_percent, eth_percent = await market_api.get_percentage_change()
            await message.channel.send(
                embed=embedder("24HR Percentages",
                               "SAFEMOON: " + sfm_percent + "\n" + "BTC: " + btc_percent + "\n" + "BNB: "
                               + bnb_percent + "\n" + "ETH: " + eth_percent))

        elif "$transfer" in message.content or "$injection" in message.content:
            remaining = price_formatter(await wallet.liquidity_wallet_transfer())
            await message.channel.send(
                embed=embedder("Remaining Tokens For Automated Liquidity Transfer", remaining + " tokens")
            )
